...
Confirm firmware version `0.4.2`. Save the complete serial output, including the decoder benchmark, spindle periods, pattern rows, drive statistics, and final summaries. `mfmbench` does no disk I/O: it measures decoding of seven patterns on the actual CPU, verifies the decoded data, and reports pulses per second and decoder time as a percentage of encoded flux duration. That percentage excludes GPIO, DMA handling, and other application work; the physical pattern test checks the combined receive path.

Use `test-control 10` to investigate unexpected disk-change errors without writing the disk. It homes the drive, then repeats head selection, a seek to cylinder 79, a one-second motor stop, and restart with a seek to cylinder zero on both heads. It reports media generation and raw control-line levels at each stage, and polls DISK_CHANGE every 50 microseconds during the motor-off interval. Any sampled LOW or generation change fails the test; keep the disk inserted throughout. Polling can miss shorter pulses and does not measure analogue signal quality. Blocking seek and motor-start operations are checked at their return, not continuously traced. The command stops and deselects the drive on completion, and refuses to run with a pending file commit.

Pattern-test failures report the failed operation and current GPIO levels before cleanup. The probe stage distinguishes seeking/motor qualification, side selection, capture startup with generation validation, and active capture. A current HIGH cannot rule out an earlier LOW that advanced the media generation. Media-change protection remains active.
