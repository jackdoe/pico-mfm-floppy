#include "sim_floppy.h"
#include "hardware/gpio.h"

static pio_sim_drive_t drive;
static floppy_t floppy;
extern uint64_t pico_test_time_us;

static disk_err_t read_sector(uint32_t expected_generation, uint8_t cylinder,
                              uint8_t head, uint8_t sector, uint8_t *data) {
  track_t track = {.cylinder = cylinder, .head = head};
  disk_err_t status = floppy_read_track(&floppy, expected_generation, &track);
  if (status == DISK_OK) memcpy(data, track.data[sector], DISK_SECTOR_SIZE);
  return status;
}

static void write_le32(uint8_t *bytes, uint32_t value) {
  bytes[0] = (uint8_t)value;
  bytes[1] = (uint8_t)(value >> 8u);
  bytes[2] = (uint8_t)(value >> 16u);
  bytes[3] = (uint8_t)(value >> 24u);
}

static void scp_checksum(uint8_t *image, size_t size) {
  uint32_t checksum = 0;
  for (size_t i = 0x10u; i < size; i++) checksum += image[i];
  write_le32(image + 0x0Cu, checksum);
}

static size_t make_minimal_scp(uint8_t *image, size_t capacity) {
  size_t required = 0x10u + 168u * 4u + DISK_TRACK_COUNT * 18u;
  ASSERT(capacity >= required);
  memset(image, 0, required);
  image[0] = 'S';
  image[1] = 'C';
  image[2] = 'P';
  image[5] = 1;
  image[7] = DISK_TRACK_COUNT - 1u;
  image[8] = 1;
  size_t position = 0x10u + 168u * 4u;
  for (uint16_t track = 0; track < DISK_TRACK_COUNT; track++) {
    ASSERT(position <= UINT32_MAX);
    write_le32(image + 0x10u + (size_t)track * 4u, (uint32_t)position);
    image[position] = 'T';
    image[position + 1u] = 'R';
    image[position + 2u] = 'K';
    image[position + 3u] = (uint8_t)track;
    write_le32(image + position + 4u, 100u);
    write_le32(image + position + 8u, 1u);
    write_le32(image + position + 12u, 16u);
    image[position + 16u] = 0;
    image[position + 17u] = 100;
    position += 18u;
  }
  scp_checksum(image, required);
  return required;
}

static void install_track(uint8_t physical_cylinder, uint8_t physical_head,
                          uint8_t claimed_cylinder, uint8_t claimed_head,
                          uint8_t salt) {
  static uint8_t pulses[PIO_SIM_MAX_FLUX];
  track_t track;
  sim_fill_track(&track, claimed_cylinder, claimed_head, salt);
  size_t count = sim_track_pulses(&track, pulses, sizeof(pulses));
  sim_install_pulses(&drive, physical_cylinder, physical_head, pulses, count);
}

TEST(test_init_configures_hd_open_drain) {
  sim_setup(&drive, &floppy);
  ASSERT(drive.density_hd);
  ASSERT_EQ(floppy_init(NULL, sim_test_pins()), DISK_ERR_INVALID);
}

TEST(test_public_read_entry_points_reject_null_and_uninitialized_instances) {
  track_t track = {.cylinder = 0, .head = 0};
  floppy_t empty = {0};
  ASSERT_EQ(floppy_read_track(NULL, 0, &track), DISK_ERR_INVALID);
  ASSERT_EQ(floppy_read_track(&empty, 0, &track), DISK_ERR_INVALID);
  disk_device_t device = floppy_device(NULL);
  ASSERT_EQ(device.read_track(device.ctx, 0, 0, 0, &track),
            DISK_ERR_INVALID);
}

TEST(test_uninitialized_automatic_context_initializes) {
  ASSERT_EQ(floppy_deinit(&floppy), DISK_OK);
  floppy_t automatic;
  pio_sim_install(&drive, sim_test_pins(), automatic.flux_ring, &automatic.ring_cpu);
  ASSERT_EQ(floppy_init(&automatic, sim_test_pins()), DISK_OK);
  ASSERT_EQ(floppy_init(&automatic, sim_test_pins()), DISK_ERR_BUSY);
  ASSERT_EQ(floppy_deinit(&automatic), DISK_OK);
  pio_sim_install(&drive, sim_test_pins(), floppy.flux_ring, &floppy.ring_cpu);
}

TEST(test_scp_loader_rejects_checksum_and_nonstandard_width) {
  uint8_t image[0x10u + 168u * 4u + DISK_TRACK_COUNT * 18u];
  size_t image_size = make_minimal_scp(image, sizeof(image));
  pio_sim_drive_t isolated;
  pio_sim_init(&isolated);
  ASSERT(pio_sim_load_scp(&isolated, image, image_size));
  image[image_size - 1u] ^= 1u;
  ASSERT(!pio_sim_load_scp(&isolated, image, image_size));
  image[image_size - 1u] ^= 1u;
  scp_checksum(image, image_size);
  image[9] = 1;
  ASSERT(!pio_sim_load_scp(&isolated, image, image_size));
  image[9] = 0;
  image[image_size - 1u] = 0;
  scp_checksum(image, image_size);
  ASSERT(!pio_sim_load_scp(&isolated, image, image_size));
  image[image_size - 1u] = 100;
  write_le32(image + 0x10u, 1u);
  scp_checksum(image, image_size);
  ASSERT(!pio_sim_load_scp(&isolated, image, image_size));
  pio_sim_free(&isolated);
}

static void assert_no_resources(void) {
  ASSERT_EQ(drive.programs_loaded, 0);
  ASSERT_EQ(drive.sms_claimed, 0);
  ASSERT_EQ(drive.dma_channels_claimed, 0);
}

TEST(test_distinct_hardware_owners_are_exclusive_and_nonmutating) {
  sim_setup(&drive, &floppy);
  floppy_t other;
  uint8_t before[sizeof(other)];
  memset(&other, 0xA5, sizeof(other));
  memcpy(before, &other, sizeof(other));

  ASSERT_EQ(floppy_init(&other, sim_test_pins()), DISK_ERR_BUSY);
  ASSERT_MEM_EQ(&other, before, sizeof(other));
  ASSERT_EQ(floppy_deinit(&other), DISK_ERR_INVALID);
  ASSERT_MEM_EQ(&other, before, sizeof(other));
  floppy_stats_t stats;
  ASSERT_EQ(floppy_stats(&floppy, &stats), DISK_OK);

  ASSERT_EQ(floppy_deinit(&floppy), DISK_OK);
  assert_no_resources();
  pio_sim_install(&drive, sim_test_pins(), other.flux_ring, &other.ring_cpu);
  ASSERT_EQ(floppy_init(&other, sim_test_pins()), DISK_OK);
  ASSERT_EQ(floppy_stats(&other, &stats), DISK_OK);
  ASSERT_EQ(floppy_deinit(&other), DISK_OK);
  assert_no_resources();
  pio_sim_install(&drive, sim_test_pins(), floppy.flux_ring, &floppy.ring_cpu);
}

TEST(test_init_deinit_and_resource_failures_are_leak_free) {
  sim_setup(&drive, &floppy);
  ASSERT_EQ(drive.programs_loaded, 2);
  ASSERT_EQ(drive.sms_claimed, 2);
  ASSERT_EQ(drive.dma_channels_claimed, 1);
  ASSERT_EQ(floppy_init(&floppy, sim_test_pins()), DISK_ERR_BUSY);
  ASSERT_EQ(drive.programs_loaded, 2);
  uint32_t before_deinit = drive.gpio_deinits;
  ASSERT_EQ(floppy_deinit(&floppy), DISK_OK);
  ASSERT_EQ(drive.gpio_deinits - before_deinit, 13);
  ASSERT_EQ(floppy_init(&floppy, sim_test_pins()), DISK_OK);
  ASSERT_EQ(drive.programs_loaded, 2);
  ASSERT_EQ(drive.sms_claimed, 2);
  ASSERT_EQ(drive.dma_channels_claimed, 1);
  ASSERT_EQ(floppy_deinit(&floppy), DISK_OK);
  assert_no_resources();
  ASSERT_EQ(floppy_deinit(&floppy), DISK_ERR_INVALID);

  drive.fail_pio_capacity = true;
  ASSERT_EQ(floppy_init(&floppy, sim_test_pins()), DISK_ERR_IO);
  assert_no_resources();
  drive.fail_pio_capacity = false;

  drive.program_add_calls = 0;
  drive.fail_program_add_call = 1;
  ASSERT_EQ(floppy_init(&floppy, sim_test_pins()), DISK_ERR_IO);
  assert_no_resources();

  drive.program_add_calls = 0;
  drive.fail_program_add_call = 2;
  ASSERT_EQ(floppy_init(&floppy, sim_test_pins()), DISK_ERR_IO);
  assert_no_resources();
  drive.fail_program_add_call = 0;

  drive.program_add_calls = 0;
  drive.sm_claim_calls = 0;
  drive.fail_sm_claim_call = 1;
  ASSERT_EQ(floppy_init(&floppy, sim_test_pins()), DISK_ERR_IO);
  assert_no_resources();

  drive.program_add_calls = 0;
  drive.sm_claim_calls = 0;
  drive.fail_sm_claim_call = 2;
  ASSERT_EQ(floppy_init(&floppy, sim_test_pins()), DISK_ERR_IO);
  assert_no_resources();
  drive.fail_sm_claim_call = 0;

  drive.program_add_calls = 0;
  drive.sm_claim_calls = 0;
  drive.fail_dma_claim = true;
  ASSERT_EQ(floppy_init(&floppy, sim_test_pins()), DISK_ERR_IO);
  assert_no_resources();
  drive.fail_dma_claim = false;

  drive.program_add_calls = 0;
  drive.sm_claim_calls = 0;
  drive.fail_read_program_init = true;
  ASSERT_EQ(floppy_init(&floppy, sim_test_pins()), DISK_ERR_IO);
  assert_no_resources();
  drive.fail_read_program_init = false;

  drive.program_add_calls = 0;
  drive.sm_claim_calls = 0;
  drive.fail_write_program_init = true;
  ASSERT_EQ(floppy_init(&floppy, sim_test_pins()), DISK_ERR_IO);
  assert_no_resources();
  drive.fail_write_program_init = false;

  drive.pio_gpio_base = 16;
  ASSERT_EQ(floppy_init(&floppy, sim_test_pins()), DISK_ERR_INVALID);
  assert_no_resources();
  drive.pio_gpio_base = 0;

  floppy_pins_t pins = sim_test_pins();
  pins.density = pins.side_select;
  ASSERT_EQ(floppy_init(&floppy, pins), DISK_ERR_INVALID);
  assert_no_resources();
  pins = sim_test_pins();
  pins.density = NUM_BANK0_GPIOS;
  ASSERT_EQ(floppy_init(&floppy, pins), DISK_ERR_INVALID);
  assert_no_resources();

  _Alignas(floppy_t) uint8_t storage[sizeof(floppy_t) + _Alignof(floppy_t)];
  floppy_t *misaligned = (floppy_t *)(void *)(storage + 1u);
  ASSERT_EQ(floppy_init(misaligned, sim_test_pins()), DISK_ERR_INVALID);
}

TEST(test_read_track_uses_zero_based_index) {
  sim_setup(&drive, &floppy);
  install_track(0, 0, 0, 0, 0x20);
  uint8_t data[DISK_SECTOR_SIZE];
  ASSERT_EQ(read_sector(sim_generation(&floppy),
                               0, 0, 0, data), DISK_OK);
  ASSERT_EQ(data[0], 0x20);
  ASSERT_EQ(data[511], (uint8_t)(0x20 + 511u));
}

TEST(test_read_track_is_one_complete_revolution) {
  sim_setup(&drive, &floppy);
  install_track(4, 1, 4, 1, 0x31);
  track_t track = {.cylinder = 4, .head = 1};
  ASSERT_EQ(floppy_read_track(&floppy, sim_generation(&floppy), &track),
            DISK_OK);
  ASSERT_EQ(track.valid, DISK_TRACK_VALID);
  for (uint8_t sector = 0; sector < DISK_SECTORS_PER_TRACK; sector++) {
    ASSERT_EQ(track.data[sector][0], (uint8_t)(0x31 + sector * 17u));
  }
}

TEST(test_seeded_drive_noise_reaches_the_hardware_path) {
  sim_setup(&drive, &floppy);
  install_track(6, 1, 6, 1, 0x4D);
  ASSERT(pio_sim_set_noise(&drive, (flux_noise_config_t){
      .seed = UINT32_C(0x10293847),
      .jitter_ticks = 3,
      .drift_ppm = 15000,
      .wander_step_ppm = 250,
      .wander_limit_ppm = 8000,
      .wander_period = 256,
  }));
  track_t expected;
  sim_fill_track(&expected, 6, 1, 0x4D);
  track_t actual = {.cylinder = 6, .head = 1};
  ASSERT_EQ(floppy_read_track(&floppy, sim_generation(&floppy), &actual), DISK_OK);
  ASSERT_EQ(actual.valid, DISK_TRACK_VALID);
  ASSERT_MEM_EQ(actual.data, expected.data, sizeof(actual.data));
}

TEST(test_track_read_preserves_crc_valid_partial_sectors) {
  sim_setup(&drive, &floppy);
  static uint8_t pulses[PIO_SIM_MAX_FLUX];
  track_t source;
  sim_fill_track(&source, 11, 0, 0x91);
  mfm_encode_t encoder;
  mfm_encode_init(&encoder, pulses, sizeof(pulses));
  mfm_encode_gap(&encoder, 80);
  for (uint8_t sector = 0; sector + 1u < DISK_SECTORS_PER_TRACK; sector++) {
    mfm_encode_sector(&encoder, 11, 0, sector, source.data[sector]);
    mfm_encode_gap(&encoder, 54);
  }
  sim_install_pulses(&drive, 11, 0, pulses, encoder.pos);
  track_t track = {.cylinder = 11, .head = 0};
  ASSERT_EQ(floppy_read_track(&floppy, sim_generation(&floppy), &track),
            DISK_ERR_TIMEOUT);
  ASSERT_EQ(track.valid,
            DISK_TRACK_VALID & ~(1u << (DISK_SECTORS_PER_TRACK - 1u)));
  for (uint8_t sector = 0; sector + 1u < DISK_SECTORS_PER_TRACK; sector++) {
    ASSERT_MEM_EQ(track.data[sector], source.data[sector], DISK_SECTOR_SIZE);
  }
}

TEST(test_stray_wrong_id_does_not_abort_requested_read) {
  sim_setup(&drive, &floppy);
  static uint8_t pulses[PIO_SIM_MAX_FLUX];
  uint8_t stray[DISK_SECTOR_SIZE];
  memset(stray, 0xE1, sizeof(stray));
  track_t correct;
  sim_fill_track(&correct, 8, 0, 0x44);
  mfm_encode_t encoder;
  mfm_encode_init(&encoder, pulses, sizeof(pulses));
  mfm_encode_gap(&encoder, 80);
  mfm_encode_sector(&encoder, 9, 0, 0, stray);
  mfm_encode_gap(&encoder, 54);
  for (uint8_t sector = 0; sector < DISK_SECTORS_PER_TRACK; sector++) {
    mfm_encode_sector(&encoder, 8, 0, sector, correct.data[sector]);
    mfm_encode_gap(&encoder, 54);
  }
  sim_install_pulses(&drive, 8, 0, pulses, encoder.pos);
  uint8_t data[DISK_SECTOR_SIZE];
  ASSERT_EQ(read_sector(sim_generation(&floppy),
                               8, 0, 0, data), DISK_OK);
  ASSERT_EQ(data[0], 0x44);
}

TEST(test_duplicate_id_in_one_revolution_is_corrupt) {
  sim_setup(&drive, &floppy);
  static uint8_t pulses[PIO_SIM_MAX_FLUX];
  track_t source;
  sim_fill_track(&source, 12, 0, 0x53);
  uint8_t stale[DISK_SECTOR_SIZE];
  memset(stale, 0xEE, sizeof(stale));
  mfm_encode_t encoder;
  mfm_encode_init(&encoder, pulses, sizeof(pulses));
  mfm_encode_gap(&encoder, 80);
  for (uint8_t sector = 0; sector < DISK_SECTORS_PER_TRACK; sector++) {
    mfm_encode_sector(&encoder, 12, 0, sector, source.data[sector]);
    mfm_encode_gap(&encoder, 54);
  }
  mfm_encode_sector(&encoder, 12, 0, 0, stale);
  mfm_encode_gap(&encoder, 54);
  sim_install_pulses(&drive, 12, 0, pulses, encoder.pos);
  track_t track = {.cylinder = 12, .head = 0};
  ASSERT_EQ(floppy_read_track(&floppy, sim_generation(&floppy), &track), DISK_ERR_CORRUPT);
  ASSERT((track.valid & 1u) == 0);
}

TEST(test_wrong_ch_does_not_mask_requested_partial_track) {
  sim_setup(&drive, &floppy);
  static uint8_t pulses[PIO_SIM_MAX_FLUX];
  track_t source;
  sim_fill_track(&source, 13, 0, 0x64);
  uint8_t stray[DISK_SECTOR_SIZE];
  memset(stray, 0xD4, sizeof(stray));
  mfm_encode_t encoder;
  mfm_encode_init(&encoder, pulses, sizeof(pulses));
  mfm_encode_gap(&encoder, 80);
  mfm_encode_sector(&encoder, 14, 0, 0, stray);
  mfm_encode_gap(&encoder, 54);
  for (uint8_t sector = 0; sector + 1u < DISK_SECTORS_PER_TRACK; sector++) {
    mfm_encode_sector(&encoder, 13, 0, sector, source.data[sector]);
    mfm_encode_gap(&encoder, 54);
  }
  sim_install_pulses(&drive, 13, 0, pulses, encoder.pos);
  track_t track = {.cylinder = 13, .head = 0};
  ASSERT_EQ(floppy_read_track(&floppy, sim_generation(&floppy), &track), DISK_ERR_TIMEOUT);
  ASSERT_EQ(track.valid,
            DISK_TRACK_VALID & ~(1u << (DISK_SECTORS_PER_TRACK - 1u)));
}

TEST(test_conflicting_data_across_revolutions_is_corrupt) {
  sim_setup(&drive, &floppy);
  static uint8_t base_pulses[100000];
  static uint8_t alternate_pulses[100000];
  track_t source;
  sim_fill_track(&source, 15, 0, 0x85);
  mfm_encode_t base;
  mfm_encode_init(&base, base_pulses, sizeof(base_pulses));
  mfm_encode_gap(&base, 80);
  for (uint8_t sector = 0; sector < 9; sector++) {
    mfm_encode_sector(&base, 15, 0, sector, source.data[sector]);
    mfm_encode_gap(&base, 54);
  }
  sim_install_pulses(&drive, 15, 0, base_pulses, base.pos);

  uint8_t conflicting[DISK_SECTOR_SIZE];
  memset(conflicting, 0xCF, sizeof(conflicting));
  mfm_encode_t alternate;
  mfm_encode_init(&alternate, alternate_pulses, sizeof(alternate_pulses));
  mfm_encode_gap(&alternate, 80);
  mfm_encode_sector(&alternate, 15, 0, 0, conflicting);
  mfm_encode_gap(&alternate, 54);
  for (uint8_t sector = 9; sector < DISK_SECTORS_PER_TRACK; sector++) {
    mfm_encode_sector(&alternate, 15, 0, sector, source.data[sector]);
    mfm_encode_gap(&alternate, 54);
  }
  sim_track_load(&drive.alternate_track, alternate_pulses, alternate.pos);
  drive.alternate_read = true;
  track_t track = {.cylinder = 15, .head = 0};
  ASSERT_EQ(floppy_read_track(&floppy, sim_generation(&floppy), &track), DISK_ERR_CORRUPT);
  ASSERT((track.valid & 1u) == 0);
}

TEST(test_wrong_track_is_exact_terminal_status) {
  sim_setup(&drive, &floppy);
  install_track(5, 0, 7, 0, 0x50);
  uint8_t data[DISK_SECTOR_SIZE];
  ASSERT_EQ(read_sector(sim_generation(&floppy),
                               5, 0, 0, data), DISK_ERR_WRONG_TRACK);
  ASSERT_EQ(floppy.stats.wrong_track, 1);
}

TEST(test_persistent_wrong_track_rehomes_and_recovers) {
  sim_setup(&drive, &floppy);
  install_track(5, 0, 5, 0, 0x75);
  install_track(7, 0, 7, 0, 0x77);
  floppy.cylinder = 5;
  floppy.track0_confirmed = true;
  drive.head_track = 7;
  uint8_t data[DISK_SECTOR_SIZE];
  ASSERT_EQ(read_sector(sim_generation(&floppy), 5, 0, 0, data), DISK_OK);
  ASSERT_EQ(data[0], 0x75);
  ASSERT(floppy.stats.recovered > 0);
}

TEST(test_wrong_side_is_exact_terminal_status) {
  sim_setup(&drive, &floppy);
  install_track(6, 0, 6, 1, 0x60);
  uint8_t data[DISK_SECTOR_SIZE];
  ASSERT_EQ(read_sector(sim_generation(&floppy),
                               6, 0, 0, data), DISK_ERR_WRONG_SIDE);
  ASSERT_EQ(floppy.stats.wrong_side, 1);
}

TEST(test_media_generation_is_edge_triggered) {
  sim_setup(&drive, &floppy);
  ASSERT_EQ(sim_generation(&floppy), 0);
  ASSERT_EQ(floppy_select(&floppy, true), DISK_OK);
  ASSERT_EQ(sim_generation(&floppy), 0);
  drive.disk_changed = true;
  ASSERT_EQ(sim_generation(&floppy), 1);
  ASSERT_EQ(sim_generation(&floppy), 1);
  drive.disk_changed = false;
  ASSERT_EQ(sim_generation(&floppy), 1);
  drive.disk_changed = true;
  ASSERT_EQ(sim_generation(&floppy), 2);
  ASSERT_EQ(floppy.stats.media_changes, 2);
}

TEST(test_media_generation_skips_zero_on_wrap) {
  sim_setup(&drive, &floppy);
  floppy.media_generation = UINT32_MAX;
  floppy.disk_change_active = false;
  ASSERT_EQ(floppy_select(&floppy, true), DISK_OK);
  ASSERT_EQ(sim_generation(&floppy), UINT32_MAX);
  drive.disk_changed = true;
  ASSERT_EQ(sim_generation(&floppy), 1);
}

TEST(test_media_latch_is_cleared_and_spindle_is_qualified_per_generation) {
  sim_setup(&drive, &floppy);
  install_track(0, 0, 0, 0, 0x68);
  ASSERT_EQ(floppy_select(&floppy, true), DISK_OK);
  ASSERT_EQ(sim_generation(&floppy), 0);
  drive.disk_changed = true;
  uint32_t first = sim_generation(&floppy);
  ASSERT_EQ(first, 1);
  uint8_t data[DISK_SECTOR_SIZE];
  ASSERT_EQ(read_sector(first, 0, 0, 0, data), DISK_OK);
  ASSERT(!drive.disk_changed);
  ASSERT(floppy.motor_qualified);
  ASSERT_EQ(floppy.motor_generation, first);
  drive.disk_changed = true;
  uint32_t second = sim_generation(&floppy);
  ASSERT_EQ(second, 2);
  ASSERT(!floppy.motor_qualified);
  uint8_t cylinder;
  ASSERT_EQ(floppy_current_track(&floppy, &cylinder), DISK_ERR_NO_TRACK0);
  ASSERT_EQ(read_sector(second, 0, 0, 0, data), DISK_OK);
  ASSERT_EQ(floppy.motor_generation, second);
}

TEST(test_disk_change_output_is_gated_by_drive_selection) {
  sim_setup(&drive, &floppy);
  drive.disk_changed = true;
  bool changed = true;
  ASSERT_EQ(floppy_disk_changed(&floppy, &changed), DISK_OK);
  ASSERT(!changed);
  ASSERT_EQ(sim_generation(&floppy), 0);
  ASSERT_EQ(floppy_select(&floppy, true), DISK_OK);
  ASSERT_EQ(floppy_disk_changed(&floppy, &changed), DISK_OK);
  ASSERT(changed);
  ASSERT_EQ(sim_generation(&floppy), 0);
  ASSERT_EQ(floppy.stats.media_changes, 0);
}

TEST(test_rx_overrun_is_explicit_and_restart_has_no_stale_data) {
  sim_setup(&drive, &floppy);
  install_track(2, 0, 2, 0, 0x72);
  uint8_t data[DISK_SECTOR_SIZE];
  uint32_t expected_generation = sim_generation(&floppy);
  drive.rx_burst_words = FLOPPY_FLUX_RING_WORDS + 32u;
  drive.rx_burst_repeat = true;
  ASSERT_EQ(read_sector(expected_generation, 2, 0, 0, data),
            DISK_ERR_OVERRUN);
  ASSERT(drive.dma_high_priority);
  ASSERT(floppy.stats.overruns > 0);
  drive.rx_burst_words = 0;
  drive.rx_burst_repeat = false;
  ASSERT_EQ(read_sector(expected_generation, 2, 0, 0, data), DISK_OK);
  ASSERT_EQ(data[0], 0x72);
}

TEST(test_transient_rx_overrun_recovers_from_fresh_stream) {
  sim_setup(&drive, &floppy);
  install_track(3, 0, 3, 0, 0x83);
  uint8_t data[DISK_SECTOR_SIZE];
  drive.rx_burst_words = FLOPPY_FLUX_RING_WORDS + 32u;
  drive.rx_burst_repeat = false;
  ASSERT_EQ(read_sector(sim_generation(&floppy),
                               3, 0, 0, data), DISK_OK);
  ASSERT_EQ(data[0], 0x83);
  ASSERT_EQ(floppy.stats.recovered, 1);
}

TEST(test_pio_rx_stall_is_explicit_and_recoverable) {
  sim_setup(&drive, &floppy);
  install_track(3, 0, 3, 0, 0x8A);
  uint8_t data[DISK_SECTOR_SIZE];
  drive.rx_pio_stall = true;
  ASSERT_EQ(read_sector(sim_generation(&floppy), 3, 0, 0, data), DISK_OK);
  ASSERT(floppy.stats.overruns > 0);
  ASSERT(floppy.stats.recovered > 0);
  drive.rx_pio_stall = true;
  drive.rx_pio_stall_repeat = true;
  ASSERT_EQ(read_sector(sim_generation(&floppy), 3, 0, 0, data),
            DISK_ERR_OVERRUN);
}

TEST(test_media_change_during_flux_is_linearized) {
  sim_setup(&drive, &floppy);
  install_track(4, 0, 4, 0, 0x9B);
  uint32_t expected = sim_generation(&floppy);
  drive.disk_change_after_samples = drive.flux_sample_reads + 100u;
  uint8_t data[DISK_SECTOR_SIZE];
  ASSERT_EQ(read_sector(expected, 4, 0, 0, data),
            DISK_ERR_MEDIA_CHANGED);
}

TEST(test_seek_rejects_invalid_cylinder_without_moving) {
  sim_setup(&drive, &floppy);
  ASSERT_EQ(floppy_seek(&floppy, 10), DISK_OK);
  ASSERT_EQ(floppy_seek(&floppy, DISK_CYLINDERS), DISK_ERR_WRONG_TRACK);
  uint8_t cylinder;
  ASSERT_EQ(floppy_current_track(&floppy, &cylinder), DISK_OK);
  ASSERT_EQ(cylinder, 10);
}

TEST(test_seek_applies_final_head_settle) {
  sim_setup(&drive, &floppy);
  ASSERT_EQ(floppy_seek(&floppy, 10), DISK_OK);
  uint64_t before = pico_test_time_us;
  ASSERT_EQ(floppy_seek(&floppy, 11), DISK_OK);
  ASSERT(pico_test_time_us - before >= 26000u);
}

TEST(test_motor_and_select_are_idempotent) {
  sim_setup(&drive, &floppy);
  ASSERT_EQ(floppy_motor_on(&floppy), DISK_OK);
  ASSERT_EQ(floppy_motor_on(&floppy), DISK_OK);
  ASSERT(floppy.motor_on);
  ASSERT_EQ(floppy_motor_off(&floppy), DISK_OK);
  ASSERT_EQ(floppy_motor_off(&floppy), DISK_OK);
  ASSERT(!floppy.motor_on);
  ASSERT_EQ(floppy_select(&floppy, true), DISK_OK);
  ASSERT_EQ(floppy_select(&floppy, true), DISK_OK);
  ASSERT(floppy.selected);
  ASSERT_EQ(floppy_select(&floppy, false), DISK_OK);
  ASSERT_EQ(floppy_select(&floppy, false), DISK_OK);
  ASSERT(!floppy.selected);
}

TEST(test_motor_rejects_rapid_index_noise) {
  sim_setup(&drive, &floppy);
  drive.index_period_us = 10000u;
  drive.index_low_us = 2000u;
  ASSERT_EQ(floppy_motor_on(&floppy), DISK_ERR_TIMEOUT);
  ASSERT(!floppy.motor_on);
}

TEST(test_raw_flux_session_is_generation_guarded) {
  sim_setup(&drive, &floppy);
  install_track(0, 0, 0, 0, 0xA0);
  uint32_t expected_generation = sim_generation(&floppy);
  ASSERT_EQ(floppy_side_select(&floppy, 0), DISK_OK);
  ASSERT_EQ(floppy_flux_begin(&floppy, expected_generation), DISK_OK);
  ASSERT_EQ(floppy_flux_begin(&floppy, expected_generation), DISK_ERR_BUSY);
  uint16_t delta;
  bool index;
  ASSERT_EQ(sim_flux_next(&floppy, &delta, &index), DISK_OK);
  drive.disk_changed = true;
  ASSERT_EQ(sim_flux_next(&floppy, &delta, &index), DISK_ERR_MEDIA_CHANGED);
  ASSERT_EQ(floppy_flux_end(&floppy), DISK_OK);
  ASSERT_EQ(floppy_flux_end(&floppy), DISK_ERR_INVALID);
  ASSERT_EQ(sim_flux_next(&floppy, &delta, &index), DISK_ERR_INVALID);
}

TEST(test_raw_flux_exclusively_owns_motion_dma_and_write_gate) {
  sim_setup(&drive, &floppy);
  install_track(0, 0, 0, 0, 0xB1);
  uint32_t expected = sim_generation(&floppy);
  ASSERT_EQ(floppy_flux_begin(&floppy, expected), DISK_OK);
  track_t track;
  sim_fill_track(&track, 0, 0, 0xB2);
  ASSERT_EQ(floppy_write_track(&floppy, expected, &track), DISK_ERR_BUSY);
  ASSERT_EQ(floppy_seek(&floppy, 1), DISK_ERR_BUSY);
  ASSERT_EQ(floppy_select(&floppy, false), DISK_ERR_BUSY);
  ASSERT_EQ(floppy_deinit(&floppy), DISK_ERR_BUSY);
  ASSERT_EQ(drive.write_gate_assertions, 0);
  ASSERT_EQ(floppy_flux_end(&floppy), DISK_OK);
}

TEST(test_raw_flux_restart_resets_pc_and_blocks_idle_shutdown) {
  sim_setup(&drive, &floppy);
  install_track(0, 0, 0, 0, 0xC2);
  uint32_t expected = sim_generation(&floppy);
  uint32_t before_jumps = drive.read_restart_jumps;
  ASSERT_EQ(floppy_flux_begin(&floppy, expected), DISK_OK);
  pico_test_time_us += (uint64_t)(FLOPPY_IDLE_TIMEOUT_MS + 1u) * 1000u;
  ASSERT_EQ(floppy_poll(&floppy), DISK_OK);
  ASSERT(floppy.motor_on);
  ASSERT(floppy.selected);
  ASSERT_EQ(floppy_flux_end(&floppy), DISK_OK);
  ASSERT_EQ(floppy_poll(&floppy), DISK_OK);
  ASSERT(!floppy.motor_on);
  ASSERT(!floppy.selected);
  ASSERT_EQ(floppy_flux_begin(&floppy, expected), DISK_OK);
  ASSERT_EQ(floppy_flux_end(&floppy), DISK_OK);
  ASSERT_EQ(drive.read_restart_jumps - before_jumps, 2);
}

TEST(test_missing_track0_is_exact_and_recoverable) {
  sim_setup(&drive, &floppy);
  install_track(0, 0, 0, 0, 0xD0);
  drive.track0_missing = true;
  uint32_t steps = drive.step_pulses;
  ASSERT_EQ(floppy_seek(&floppy, 0), DISK_ERR_NO_TRACK0);
  ASSERT_EQ(drive.step_pulses - steps, 90);
  uint8_t cylinder;
  ASSERT_EQ(floppy_current_track(&floppy, &cylinder), DISK_ERR_NO_TRACK0);
  uint8_t data[DISK_SECTOR_SIZE];
  ASSERT_EQ(read_sector(sim_generation(&floppy), 0, 0, 0, data), DISK_ERR_NO_TRACK0);
  ASSERT_EQ(floppy.stats.failed, 1);
  drive.track0_missing = false;
  ASSERT_EQ(floppy_seek(&floppy, 0), DISK_OK);
  ASSERT_EQ(floppy_current_track(&floppy, &cylinder), DISK_OK);
  ASSERT_EQ(cylinder, 0);
  ASSERT_EQ(read_sector(sim_generation(&floppy), 0, 0, 0, data), DISK_OK);
  ASSERT_EQ(data[0], 0xD0);
}

TEST(test_stuck_disk_change_line_fails_without_flux_or_generation_churn) {
  sim_setup(&drive, &floppy);
  install_track(0, 0, 0, 0, 0xD5);
  ASSERT_EQ(floppy_select(&floppy, true), DISK_OK);
  drive.disk_changed = true;
  drive.disk_change_stuck = true;
  uint32_t expected = sim_generation(&floppy);
  ASSERT_EQ(expected, 0);
  uint32_t samples = drive.flux_sample_reads;
  uint8_t data[DISK_SECTOR_SIZE];
  ASSERT_EQ(read_sector(expected, 0, 0, 0, data), DISK_ERR_MEDIA_CHANGED);
  ASSERT_EQ(read_sector(expected, 0, 0, 0, data), DISK_ERR_MEDIA_CHANGED);
  ASSERT_EQ(drive.flux_sample_reads, samples);
  ASSERT_EQ(sim_generation(&floppy), 0);
  ASSERT_EQ(floppy.stats.media_changes, 0);
  drive.disk_change_stuck = false;
  ASSERT_EQ(read_sector(sim_generation(&floppy), 0, 0, 0, data), DISK_OK);
  ASSERT_EQ(data[0], 0xD5);
}

TEST(test_observers_during_raw_flux_are_typed) {
  sim_setup(&drive, &floppy);
  install_track(0, 0, 0, 0, 0xD3);
  ASSERT_EQ(floppy_side_select(&floppy, DISK_HEADS), DISK_ERR_INVALID);
  ASSERT_EQ(floppy_seek(&floppy, 3), DISK_OK);
  bool active = true;
  ASSERT_EQ(floppy_at_track0(&floppy, &active), DISK_OK);
  ASSERT(!active);
  ASSERT_EQ(floppy_seek(&floppy, 0), DISK_OK);
  ASSERT_EQ(floppy_flux_begin(&floppy, sim_generation(&floppy)), DISK_OK);
  uint16_t delta;
  bool index;
  ASSERT_EQ(sim_flux_next(&floppy, &delta, &index), DISK_OK);
  floppy_stats_t stats;
  ASSERT_EQ(floppy_stats(&floppy, &stats), DISK_ERR_BUSY);
  ASSERT_EQ(floppy_stats_reset(&floppy), DISK_ERR_BUSY);
  ASSERT_EQ(floppy_side_select(&floppy, 1), DISK_ERR_BUSY);
  ASSERT_EQ(floppy_side_select(&floppy, DISK_HEADS), DISK_ERR_INVALID);
  ASSERT_EQ(floppy_at_track0(&floppy, &active), DISK_OK);
  ASSERT(active);
  ASSERT_EQ(floppy_flux_end(&floppy), DISK_OK);
  ASSERT_EQ(floppy_stats(&floppy, &stats), DISK_OK);
  ASSERT(stats.flux_words > 0);
  ASSERT_EQ(floppy_stats_reset(&floppy), DISK_OK);
  ASSERT_EQ(floppy_stats(&floppy, &stats), DISK_OK);
  ASSERT_EQ(stats.flux_words, 0);
}

TEST(test_latch_found_on_first_selection_is_initial_media_state) {
  sim_setup(&drive, &floppy);
  install_track(0, 0, 0, 0, 0xD7);
  drive.disk_changed = true;
  uint32_t expected = sim_generation(&floppy);
  ASSERT_EQ(expected, 0);
  uint8_t data[DISK_SECTOR_SIZE];
  ASSERT_EQ(read_sector(expected, 0, 0, 0, data), DISK_OK);
  ASSERT_EQ(data[0], 0xD7);
  ASSERT(!drive.disk_changed);
  ASSERT_EQ(sim_generation(&floppy), 0);
  ASSERT_EQ(floppy.stats.media_changes, 0);
  drive.disk_changed = true;
  ASSERT_EQ(read_sector(expected, 0, 0, 0, data), DISK_ERR_MEDIA_CHANGED);
  ASSERT_EQ(sim_generation(&floppy), 1);
  ASSERT_EQ(floppy.stats.media_changes, 1);
}

TEST(test_control_operations_adopt_swapped_media_and_data_operations_do_not) {
  sim_setup(&drive, &floppy);
  install_track(0, 0, 0, 0, 0xD9);
  uint8_t data[DISK_SECTOR_SIZE];
  uint32_t before = sim_generation(&floppy);
  ASSERT_EQ(read_sector(before, 0, 0, 0, data), DISK_OK);
  drive.disk_changed = true;
  ASSERT_EQ(floppy_seek(&floppy, 3), DISK_OK);
  ASSERT(!drive.disk_changed);
  uint32_t after = sim_generation(&floppy);
  ASSERT_EQ(after, before + 1u);
  ASSERT_EQ(floppy.stats.media_changes, 1);
  ASSERT_EQ(read_sector(before, 0, 0, 0, data), DISK_ERR_MEDIA_CHANGED);
  ASSERT_EQ(read_sector(after, 0, 0, 0, data), DISK_OK);
  ASSERT_EQ(data[0], 0xD9);
  ASSERT_EQ(floppy_motor_off(&floppy), DISK_OK);
  drive.disk_changed = true;
  ASSERT_EQ(floppy_motor_on(&floppy), DISK_OK);
  ASSERT_EQ(sim_generation(&floppy), after + 1u);
  ASSERT_EQ(floppy_seek(&floppy, 0), DISK_OK);
  ASSERT_EQ(read_sector(sim_generation(&floppy), 0, 0, 0, data), DISK_OK);
}

TEST(test_raw_flux_batches_match_single_pulses) {
  sim_setup(&drive, &floppy);
  install_track(0, 0, 0, 0, 0xDB);
  enum { PULSES = 3000, BATCH = 7 };
  static floppy_pulse_t single[PULSES];
  static floppy_pulse_t batched[PULSES];
  uint32_t expected = sim_generation(&floppy);
  ASSERT_EQ(floppy_flux_begin(&floppy, expected), DISK_OK);
  for (size_t i = 0; i < PULSES; i++) {
    disk_result_t result = floppy_flux_read(&floppy, &single[i], 1);
    ASSERT_EQ(result.error, DISK_OK);
    ASSERT_EQ(result.count, 1);
  }
  ASSERT_EQ(floppy_flux_end(&floppy), DISK_OK);

  ASSERT_EQ(floppy_flux_begin(&floppy, expected), DISK_OK);
  size_t total = 0;
  size_t largest = 0;
  while (total < PULSES) {
    size_t want = PULSES - total < BATCH ? PULSES - total : BATCH;
    disk_result_t result = floppy_flux_read(&floppy, batched + total, want);
    ASSERT_EQ(result.error, DISK_OK);
    ASSERT(result.count >= 1 && result.count <= want);
    if (result.count > largest) largest = result.count;
    total += result.count;
  }
  ASSERT_EQ(floppy_flux_end(&floppy), DISK_OK);
  ASSERT_EQ(largest, BATCH);
  for (size_t i = 0; i < PULSES; i++) {
    ASSERT_EQ(batched[i].delta, single[i].delta);
    ASSERT_EQ(batched[i].index, single[i].index);
  }
  ASSERT_EQ(floppy_flux_read(&floppy, batched, 1).error, DISK_ERR_INVALID);
  ASSERT_EQ(floppy_flux_begin(&floppy, expected), DISK_OK);
  ASSERT_EQ(floppy_flux_read(&floppy, batched, 0).error, DISK_ERR_INVALID);
  drive.disk_changed = true;
  disk_result_t swapped = floppy_flux_read(&floppy, batched, BATCH);
  ASSERT_EQ(swapped.error, DISK_ERR_MEDIA_CHANGED);
  ASSERT_EQ(swapped.count, 0);
  ASSERT_EQ(floppy_flux_end(&floppy), DISK_OK);
}

static void align_to_buffered_half(void) {
  while (!floppy.read.half_valid) {
    uint16_t delta;
    bool index;
    ASSERT_EQ(sim_flux_next(&floppy, &delta, &index), DISK_OK);
  }
}

TEST(test_raw_flux_batch_stops_at_lost_samples) {
  sim_setup(&drive, &floppy);
  install_track(0, 0, 0, 0, 0xDC);
  uint32_t expected = sim_generation(&floppy);
  floppy_pulse_t batch[64];
  ASSERT_EQ(floppy_flux_begin(&floppy, expected), DISK_OK);
  align_to_buffered_half();
  drive.rx_burst_words = FLOPPY_FLUX_RING_WORDS + 32u;
  disk_result_t result = floppy_flux_read(&floppy, batch, 64);
  ASSERT_EQ(result.error, DISK_ERR_OVERRUN);
  ASSERT_EQ(result.count, 1);
  ASSERT_EQ(floppy.stats.overruns, 1);
  ASSERT_EQ(floppy_flux_end(&floppy), DISK_OK);

  ASSERT_EQ(floppy_flux_begin(&floppy, expected), DISK_OK);
  align_to_buffered_half();
  drive.rx_pio_stall = true;
  result = floppy_flux_read(&floppy, batch, 64);
  ASSERT_EQ(result.error, DISK_ERR_OVERRUN);
  ASSERT_EQ(result.count, 64);
  ASSERT_EQ(floppy.stats.overruns, 2);
  ASSERT_EQ(floppy_flux_end(&floppy), DISK_OK);
}

int main(void) {
  printf("=== PIO Simulator Tests ===\n\n");
  RUN_TEST(test_init_configures_hd_open_drain);
  RUN_TEST(test_public_read_entry_points_reject_null_and_uninitialized_instances);
  RUN_TEST(test_uninitialized_automatic_context_initializes);
  RUN_TEST(test_scp_loader_rejects_checksum_and_nonstandard_width);
  RUN_TEST(test_distinct_hardware_owners_are_exclusive_and_nonmutating);
  RUN_TEST(test_init_deinit_and_resource_failures_are_leak_free);
  RUN_TEST(test_read_track_uses_zero_based_index);
  RUN_TEST(test_read_track_is_one_complete_revolution);
  RUN_TEST(test_seeded_drive_noise_reaches_the_hardware_path);
  RUN_TEST(test_track_read_preserves_crc_valid_partial_sectors);
  RUN_TEST(test_stray_wrong_id_does_not_abort_requested_read);
  RUN_TEST(test_duplicate_id_in_one_revolution_is_corrupt);
  RUN_TEST(test_wrong_ch_does_not_mask_requested_partial_track);
  RUN_TEST(test_conflicting_data_across_revolutions_is_corrupt);
  RUN_TEST(test_wrong_track_is_exact_terminal_status);
  RUN_TEST(test_persistent_wrong_track_rehomes_and_recovers);
  RUN_TEST(test_wrong_side_is_exact_terminal_status);
  RUN_TEST(test_media_generation_is_edge_triggered);
  RUN_TEST(test_media_generation_skips_zero_on_wrap);
  RUN_TEST(test_media_latch_is_cleared_and_spindle_is_qualified_per_generation);
  RUN_TEST(test_disk_change_output_is_gated_by_drive_selection);
  RUN_TEST(test_rx_overrun_is_explicit_and_restart_has_no_stale_data);
  RUN_TEST(test_transient_rx_overrun_recovers_from_fresh_stream);
  RUN_TEST(test_pio_rx_stall_is_explicit_and_recoverable);
  RUN_TEST(test_media_change_during_flux_is_linearized);
  RUN_TEST(test_seek_rejects_invalid_cylinder_without_moving);
  RUN_TEST(test_seek_applies_final_head_settle);
  RUN_TEST(test_motor_and_select_are_idempotent);
  RUN_TEST(test_motor_rejects_rapid_index_noise);
  RUN_TEST(test_raw_flux_session_is_generation_guarded);
  RUN_TEST(test_raw_flux_exclusively_owns_motion_dma_and_write_gate);
  RUN_TEST(test_raw_flux_restart_resets_pc_and_blocks_idle_shutdown);
  RUN_TEST(test_missing_track0_is_exact_and_recoverable);
  RUN_TEST(test_stuck_disk_change_line_fails_without_flux_or_generation_churn);
  RUN_TEST(test_observers_during_raw_flux_are_typed);
  RUN_TEST(test_latch_found_on_first_selection_is_initial_media_state);
  RUN_TEST(test_control_operations_adopt_swapped_media_and_data_operations_do_not);
  RUN_TEST(test_raw_flux_batches_match_single_pulses);
  RUN_TEST(test_raw_flux_batch_stops_at_lost_samples);
  ASSERT_EQ(floppy_deinit(&floppy), DISK_OK);
  pio_sim_free(&drive);
  TEST_RESULTS();
}
